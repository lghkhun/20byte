...
export function ConversationRow({ density, conversation, isSelected, onSelect }: ConversationRowProps) {
  ...
  return (
    <button
      ...
      className={
        isSelected
          ? `w-full border-l-2 border-primary bg-accent/70 px-4 sm:px-5 ${density === "compact" ? "py-2.5" : "py-3.5"} text-left transition-all duration-150`
          : `w-full border-l-2 border-transparent px-4 sm:px-5 ${density === "compact" ? "py-2.5" : "py-3.5"} text-left transition-all duration-150 hover:bg-accent/40`
      }
    >
      <div className="flex items-start gap-3">
        <div
          className={`relative flex ${density === "compact" ? "h-9 w-9 text-xs" : "h-10 w-10 text-sm"} shrink-0 items-center justify-center rounded-full font-semibold ${toAvatarTone(
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className={`${density === "compact" ? "mt-0.5" : "mt-1"} flex items-center gap-1`}>
            <span className="line-clamp-1 text-xs text-muted-foreground">{conversation.lastMessagePreview ?? "No message preview yet"}</span>
          </div>
          <div className={`${density === "compact" ? "mt-1.5" : "mt-2"} flex items-center justify-between`}>
            <span className={sourceBadge.className} title={tooltipText}>
              {sourceBadge.label}
            </span>
            {conversation.unreadCount > 0 ? (
              <span className="inline-flex min-w-6 items-center justify-center rounded-full bg-emerald-500 px-2 py-0.5 text-xs font-semibold text-white">
